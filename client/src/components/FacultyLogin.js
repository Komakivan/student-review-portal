...
const FacultyLogin = () => {
  ...
  const {loginFaculty} = useContext(facultyContext)
// const navigate = useNavigate()

const onSubmit = async  (data) => {
    await loginFaculty(data)
}
  return (
    ...
    <div className="w-full py-8">
        ...
        <div className="bg-white w-5/6 md:w-3/4 lg:w-2/3 xl:w-[500px] 2xl:w-[550px] mt-8 mx-auto px-16 py-8 rounded-lg shadow-2xl">
            ...
            <h2 className="text-center text-2xl font-bold tracking-wide text-gray-800">Faculty Sign in</h2>
            {/* <p className="text-center text-sm text-gray-600 mt-2">Don't have an account? <Link to='/register' className="text-blue-600 hover:text-blue-700 hover:underline" title="Sign In">Sign up here</Link></p> */}
            ...
            <form className="my-8 text-sm" onSubmit={handleSubmit(onSubmit)}>
                ...
                <div className="flex flex-col my-4">
                    ...
                    <input type="text" name="name" id="name" className="mt-2 p-2 border border-gray-300 focus:outline-none focus:ring-0 focus:border-gray-300 rounded text-sm text-gray-900" placeholder="Enter Email" {...register('email')}/>
                    <p className='text-xs mt-1 text-red-800'>{errors.email?.message}</p>
                </div>
                ...
                <div className="flex flex-col my-4">
                    ...
                    <div className="relative flex flex-col items-left mt-2">
                        ...
                        <p className='text-xs mt-1 text-red-800'>{errors.password?.message}</p>
                    </div>
                </div>
                <Link to='/faculty-reset-password' className='text-xs hover:underline text-blue-800 mt-1'>Forgot Password?</Link>
